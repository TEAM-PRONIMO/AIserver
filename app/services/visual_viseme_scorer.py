...
# ── 압축 landmark 인덱스 (FE 압축 배열의 0~25 위치) ────────────────────────
# FE는 MediaPipe 478점 중 26개만 골라 순서대로 보냄.
# 원본 인덱스 순서 (FE REQUIRED_LANDMARK_INDICES 와 동일):
#   [0, 13, 14, 17, 33, 42, 61, 78, 81, 82,
#    87, 88, 95, 178, 183, 263, 291, 308, 310, 311,
#    312, 317, 318, 324, 402, 415]
OUTER_UPPER_LIP = 0   # 원본 0
INNER_UPPER_LIP = 1   # 원본 13
INNER_LOWER_LIP = 2   # 원본 14
OUTER_LOWER_LIP = 3   # 원본 17
LEFT_EYE_OUTER  = 4   # 원본 33
MOUTH_LEFT      = 6   # 원본 61
RIGHT_EYE_OUTER = 15  # 원본 263
MOUTH_RIGHT     = 16  # 원본 291

# 원본 [78, 95, 88, 178, 87, 14, 317, 402, 318, 324,
#       308, 415, 310, 311, 312, 13, 82, 81, 42, 183] 를 압축 인덱스로 매핑.
INNER_LIP_LOOP = [
    7, 12, 11, 13, 10,  2, 21, 24, 22, 23,
    17, 25, 18, 19, 20,  1,  9,  8,  5, 14,
]

EXPECTED_LANDMARK_COUNT = 26
...
def extract_canonical_frame_features(
    ...
    """
    MediaPipe 1 프레임 (압축 형식) → canonical feature 17개 (0~1 정규화).

    입력 형식:
      face_landmarks   : 26개 압축 배열 (FE REQUIRED_LANDMARK_INDICES 순서).
                         원본 MediaPipe 478점 중 입/눈 주변 26개만 골라 보냄.
      face_blendshapes : 13개 mouth-region blendshape 만.

    출처:
      - blendshape 이름: MediaPipe Face Landmarker 공식 문서
      - 랜드마크 인덱스: MediaPipe Face Mesh 478점 명세에서 26개 추출
      - tongueOut: TalkingHead 오픈소스에서 TH 발음 핵심 feature로 확인
    """
    if len(face_landmarks) != EXPECTED_LANDMARK_COUNT:
        raise ValueError(
            f"face_landmarks가 {EXPECTED_LANDMARK_COUNT}점 압축 형식이 아닙니다 (got {len(face_landmarks)})."
        )
