...
def fuse_word_audio_visual(
    ...
    skip-only 단어 (모든 음소가 카메라로 측정 불가, 예: "girl"=g+er+l) 처리:
      total_weight ≈ 0 → audio 100% 가중치. visual_score 는 audio 와 동일값으로 fold.
    """
    audio_score  = float(audio_scoring.get("custom_audio_score_0_1", 0.0))
    visual_score = float(visual_scoring.get("visual_score_0_1", 0.0))
    total_weight = float(visual_scoring.get("total_weight", 0.0))

    visual_measurable = total_weight > 1e-6

    if not visual_measurable:
        # 시각 측정 불가 → audio 100%. visual 표시값도 audio 와 같게 (UI 일관성).
        audio_weight  = 1.0
        visual_weight = 0.0
    ...
    return {
        ...
        "visual_measurable": visual_measurable,
